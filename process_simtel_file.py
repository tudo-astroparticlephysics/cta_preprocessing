...
from ctapipe.image.cleaning import (
    tailcuts_clean, fact_image_cleaning, number_of_islands)
from ctapipe.image.timing_parameters import timing_parameters
from ctapipe.reco import HillasReconstructor

from preprocessing.parameters import PREPConfig
from preprocessing.containers import (
    TelescopeParameterContainer, ArrayEventContainer,
    RunInfoContainer, IslandContainer)
...
def main(
        ...
        n_events, n_jobs, overwrite, verbose):
    '''
    ...
    write_result_to_file(
        run_info_container, array_events, telescope_events, output_file)
...
def process_file(input_file, config, n_jobs=1, n_events=-1, verbose=1):
    ...
    allowed_tels = [id for id in source._subarray_info.tels
                    if source._subarray_info.tels[id].camera.cam_id
                    in config.allowed_cameras]
    source.allowed_tels = allowed_tels

    event_iterator = filter(lambda e: len(e.dl0.tels_with_data) > 1, source)

    with Parallel(
            n_jobs=n_jobs, verbose=verbose, prefer='processes') as parallel:
        # process events in parallel
        p = parallel(delayed(partial(
            process_parallel, calibrator=calibrator, config=config))
            (copy.deepcopy(e)) for e in event_iterator)
    ...
    telescope_event_containers = [item for sublist in nested_tel_events
                                  for item in sublist]

    mc_header_container = source.mc_header_information
    mc_header_container.prefix = 'mc'

    run_info_container = RunInfoContainer(
        run_id=array_event_containers[0].run_id, mc=mc_header_container)

    return (run_info_container,
            array_event_containers,
...
def process_event(event, calibrator, config):
    ...
    for telescope_id, dl1 in event.dl1.tel.items():
        telescope_types.append(
            event.inst.subarray.tels[telescope_id].optics.tel_type)
    ...
    if len(telescope_event_containers) < 2:
        raise ReconstructionError(
            'Not enough telescopes for which '
            'Hillas parameters could be reconstructed.')

    parameters = {
        tel_id: telescope_event_containers[tel_id].hillas
        for tel_id in telescope_event_containers}
    pointing_altitude = {
        tel_id: telescope_event_containers[tel_id].pointing.altitude
        for tel_id in telescope_event_containers}
    pointing_azimuth = {
        tel_id: telescope_event_containers[tel_id].pointing.azimuth
        for tel_id in telescope_event_containers}

    reconstruction_container = hillas_reconstructor.predict(
        parameters, event.inst,
        pointing_alt=pointing_altitude, pointing_az=pointing_azimuth)
    reconstruction_container.prefix = ''
    calculate_distance_to_core(
        telescope_event_containers, event, reconstruction_container)
    ...
    array_event = ArrayEventContainer(
        ...
        total_intensity=sum(
            [t.hillas.intensity
             for t in telescope_event_containers.values()]),
